---
title: "Application of Statistical Modelling and Data Mining in Prediction of Death due to Heart Attack "
...
library(ggplot2)
ggplot(HeartData_long, aes(x = variable, y = value, fill=factor(DEATH_EVENT))) +            
...
#check for outliers 
boxplot.stats(HeartData$age,coef=2)$out

 shapiro.test(HeartData$age)
 

    # Transformation
 HeartData$age<-log(HeartData$age+1)
...
ggplot()+
  ...
  ggtitle(" plot of age and dealth event")
```
...
#X-squared = 1.0422, df = 1, p-value = 0.3073, If we assume predetermined level of significance to be 0.05 then we fail to reject the null hypothesis.
...
ggplot()+
  ...
  ggtitle(" plot of creatinine_phosphokinase and dealth event")
...
#Null hypothesis: There are no relationships between the categorical variables. If you know the value of one variable, it does not help you predict the value of another variable.
#Alternative hypothesis: There are relationships between the categorical variables. Knowing the value of one variable does help you predict the value of another variable.
#X-squared = 0, df = 1, p-value = 1, If we assume predetermined level of significance to be 0.05 then we reject the null hypothesis and accept the alternate hypothesis. 
...
HeartData$ejection_fraction
 boxplot.stats(HeartData$ejection_fraction)$out
...
#Null hypothesis: There are no relationships between the categorical variables. If you know the value of one variable, it does not help you predict the value of another variable.
#Alternative hypothesis: There are relationships between the categorical variables. Knowing the value of one variable does help you predict the value of another variable.
#X-squared = 1.7684, df = 1, p-value = 0.1836, If we assume predetermined level of significance to be 0.05 then we fail to reject the null hypothesis.
...
#Null hypothesis: There are no relationships between the categorical variables. If you know the value of one variable, it does not help you predict the value of another variable.
#Alternative hypothesis: There are relationships between the categorical variables. Knowing the value of one variable does help you predict the value of another variable.
#X-squared = 0.0065762, df = 1, p-value = 0.9354, If we assume predetermined level of significance to be 0.05 then we fail to reject the null hypothesis , which means there is no statistical relationship
...
#Null hypothesis: There are no relationships between the categorical variables. If you know the value of one variable, it does not help you predict the value of another variable.
#Alternative hypothesis: There are relationships between the categorical variables. Knowing the value of one variable does help you predict the value of another variable.
#X-squared = 0.066235, df = 1, p-value = 0.7969, If we assume predetermined level of significance to be 0.05 then we fail to reject the null hypothesis, which means there is no statistical relationship.
...
table(HeartData$DEATH_EVENT)
correlations=cor(HeartData,HeartData$DEATH_EVENT,method = "spearman")
correlations 
plot(correlations)
 
corrplot(correlations, number.cex =1, method = "circle", type = "full", tl.cex=.8,tl.col = "black")
HeartDatamatrix<-as.matrix(HeartData)
Corelationmatrix<-rcorr(HeartDatamatrix)
Corelationmatrix
# Extract the correlation coefficients
Corelationmatrix$r
correlationcoef<-as.data.frame(Corelationmatrix$r)
correlationcoef$DEATH_EVENT<-as.matrix(correlationcoef$DEATH_EVENT)
correlationcoef$DEATH_EVENT
# Extract p-values
Corelationmatrix$P 
Pvalue<-as.data.frame(Corelationmatrix$P )
Pvalue$DEATH_EVENT<-as.matrix(Pvalue$DEATH_EVENT)
Pvalue$DEATH_EVENT

correlation_correlationcoef_Pvalue<-cbind(correlations,correlationcoef$DEATH_EVENT,Pvalue$DEATH_EVENT)  
...
library(dplyr)
str(HeartData)
   HeartData_IMP<-dplyr::select(HeartData,time,serum_creatinine,ejection_fraction,creatinine_phosphokinase,age,serum_sodium,platelets,DEATH_EVENT)
    HeartData_IMP4<-dplyr::select(HeartData,time,serum_creatinine,ejection_fraction,creatinine_phosphokinase,DEATH_EVENT)
    HeartData_IMP
    HeartData_IMP4
```
...
plot(HeartData_R[c("age","anaemia","creatinine_phosphokinase","diabetes","ejection_fraction","high_blood_pressure","platelets","serum_creatinine","serum_sodium","sex","smoking","time")],  
     ...
     main = "K-means with 2 clusters Cluster HeartData 12 features") 
...
clusplot(HeartData_R[, c("age","anaemia","creatinine_phosphokinase","diabetes","ejection_fraction","high_blood_pressure","platelets","serum_creatinine","serum_sodium","sex","smoking","time")], 
         ...
         plotchar = TRUE, 
         span = TRUE, 
         main = paste("Cluster HeartData 12 features"), 
...
lvs <- c("0", "1")
truth <- factor(rep(lvs, times = c(144, 176)),
                levels = rev(lvs))
kmeans.re$cluster <- factor(
               c(
                 rep(lvs, times = c(75,85)),
                 rep(lvs, times = c(69,91))),               
               levels = rev(lvs))
length(kmeans.re$cluster)
length(truth)
xtab <- table(kmeans.re$cluster, truth)
caret::confusionMatrix(xtab)
result<- confusionMatrix(kmeans.re$cluster, truth)
result
...
# Removing initial label of from original dataset 
HeartData_IMP_R<-HeartData_IMP[,-8] 
  
# Fitting K-Means clustering Model  
# to training dataset 
set.seed(240) # Setting seed 
kmeans.reimp<- kmeans(HeartData_IMP_R, centers = 2, nstart = 20) 
kmeans.reimp 
...
# Confusion Matrix 
Impconfusion_mat<- table(HeartData_IMP$DEATH_EVENT,kmeans.reimp$cluster) 
Impconfusion_mat
  
# Model Evaluation and visualization 
plot(HeartData_IMP_R[c("time","serum_creatinine","ejection_fraction","creatinine_phosphokinase","age","serum_sodium","platelets")],  
     col = kmeans.reimp$cluster,pch = 8, cex =0.5,
     main = "K-means with 2 clusters Cluster HeartData_IMP 7 important features") 
## Visualizing clusters 
n_kmeansimp<- kmeans.reimp$cluster 
clusplot(HeartData_IMP_R[, c("time","serum_creatinine","ejection_fraction","creatinine_phosphokinase","age","serum_sodium","platelets")], 
         n_kmeans, 
         lines = 1, 
         shade = TRUE, 
         color = TRUE, 
         ...
         plotchar = TRUE, 
         span = TRUE, 
         main = paste("Cluster HeartData_IMP 7 important features"), 
         xlab = 'cluster pointsx', 
         ylab = 'clusterpointy')
n_kmeansimp

lvs <- c("0", "1")
truth1<- factor(rep(lvs, times = c(160,160)),
                levels = rev(lvs))
kmeans.reimp$cluster<- factor(
               c(
                 rep(lvs, times = c(73,87)),
                 rep(lvs, times = c(68,92))),               
               levels = rev(lvs))
xtab1<-table(kmeans.reimp$cluster,truth1)
length(kmeans.reimp$cluster)
length(truth1)
#table(truth1)
caret::confusionMatrix(xtab1)
result1<- confusionMatrix(kmeans.reimp$cluster,truth1)
result1
...
# Removing initial label of from original dataset 
HeartData_IMP_R<-HeartData_IMP4[,-5] 
  
# Fitting K-Means clustering Model  
# to training dataset 
set.seed(240) # Setting seed 
kmeans.reimp4<- kmeans(HeartData_IMP_R, centers = 2, nstart = 20) 
kmeans.reimp4 
...
# Confusion Matrix 
Impconfusion_mat<- table(HeartData_IMP4$DEATH_EVENT,kmeans.reimp4$cluster) 
Impconfusion_mat
  
# Model Evaluation and visualization 
plot(HeartData_IMP_R[c("time","serum_creatinine","ejection_fraction","creatinine_phosphokinase")],  
     col = kmeans.reimp4$cluster,pch = 8, cex =0.5,
     main = "K-means with 2 clusters,Cluster HeartData_IMP4 4 most important features") 
## Visualizing clusters 
n_kmeansimp4<- kmeans.reimp4$cluster 
clusplot(HeartData_IMP_R[, c("time","serum_creatinine","ejection_fraction","creatinine_phosphokinase")], 
         n_kmeans, 
         lines = 1, 
         shade = TRUE, 
         color = TRUE, 
         ...
         plotchar = TRUE, 
         span = TRUE, 
         main = paste("Cluster HeartData_IMP4 4 most important features"), 
         xlab = 'cluster pointsx', 
         ylab = 'clusterpointy')
n_kmeansimp4

lvs <- c("0", "1")
truth2<- factor(rep(lvs, times = c(160,160)),
                levels = rev(lvs))
kmeans.reimp4$cluster<- factor(
               c(
                 ...
                 rep(lvs, times = c(68,92))),               
               levels = rev(lvs))
xtab2<-table(kmeans.reimp4$cluster,truth2)
#length(kmeans.reimp4$cluster)
#length(truth2)
caret::confusionMatrix(xtab1)
result2<- confusionMatrix(kmeans.reimp4$cluster,truth2)
result2
precision1<- result1$byClass['Pos Pred Value']
precision1
...
kmeans.reimp4<- kmeans(HeartData_IMP_R, centers = 3, nstart = 20) 
kmeans.reimp4 
...
plot(HeartData_IMP_R[c("time","serum_creatinine","ejection_fraction","creatinine_phosphokinase")],  
     col = kmeans.reimp4$cluster,pch = 8, cex =0.5,
     main = "K-means with 3 clusters,Cluster HeartData_IMP4 4 most important features") 
## Visualizing clusters 
n_kmeansimp4<- kmeans.reimp4$cluster 
clusplot(HeartData_IMP_R[, c("time","serum_creatinine","ejection_fraction","creatinine_phosphokinase")], 
         n_kmeans, 
...
kmeans.reimp4<- kmeans(HeartData_IMP_R, centers = 4, nstart = 20) 
kmeans.reimp4 
...
plot(HeartData_IMP_R[c("time","serum_creatinine","ejection_fraction","creatinine_phosphokinase")],  
     col = kmeans.reimp4$cluster,pch = 8, cex =0.5,
     main = "K-means with 4 clusters,Cluster HeartData_IMP4 4 most important features") 
## Visualizing clusters 
n_kmeansimp4<- kmeans.reimp4$cluster 
clusplot(HeartData_IMP_R[, c("time","serum_creatinine","ejection_fraction","creatinine_phosphokinase")], 
         n_kmeans, 
...
con<-table(classifier_knn, test$DEATH_EVENT)
con
# Model Evaluation - Choosing K 
# Calculate out of Sample error 
misClassError1 <- mean(classifier_knn != test$DEATH_EVENT) 
print(paste('Accuracy =', 1-misClassError1)) 

lvs <- c("0", "1")
truth<- factor(rep(lvs, times = c(32, 32)),
                levels = rev(lvs))
predict_Dealth1<- factor(
               c(
                 rep(lvs, times = c(26, 7)),
                 rep(lvs, times = c(6, 25))),               
               levels = rev(lvs))
...
# Model Evaluation - Choosing K 
# Calculate out of Sample error 
misClassError3 <- mean(classifier_knn3 != test$DEATH_EVENT) 
print(paste('Accuracy =', 1-misClassError3))

   conm3<-table(classifier_knn3,test$DEATH_EVENT)
   conm3
...
predict_Dealth1<- factor(
               c(
                 rep(lvs, times = c(22, 8)),
                 rep(lvs, times = c(10, 24))),               
               levels = rev(lvs))
xtab <- table(predict_Dealth1, truth)
caret::confusionMatrix(xtab)
result<- confusionMatrix(predict_Dealth1, truth)
...
# Model Evaluation - Choosing K 
# Calculate out of Sample error 
misClassError5<- mean(classifier_knn5 != test$DEATH_EVENT) 
print(paste('Accuracy =', 1-misClassError5)) 

table(test$DEATH_EVENT)
   conm5<-table(classifier_knn5,test$DEATH_EVENT)
   conm5
...
predict_Dealth1<- factor(
               c(
                 rep(lvs, times = c(22, 7)),
                 rep(lvs, times = c(10, 25))),               
               levels = rev(lvs))
xtab <- table(predict_Dealth1, truth)
caret::confusionMatrix(xtab)
result<- confusionMatrix(predict_Dealth1, truth)
...
# Model Evaluation - Choosing K 
# Calculate out of Sample error 
misClassError7<- mean(classifier_knn7 != test$DEATH_EVENT) 
print(paste('Accuracy =', 1-misClassError7))

   conm7<-table(classifier_knn7,test$DEATH_EVENT)
   conm7
...
predict_Dealth1<- factor(
               c(
                 rep(lvs, times = c(24, 8)),
                 rep(lvs, times = c(8, 24))),               
               levels = rev(lvs))
xtab <- table(predict_Dealth1, truth)
caret::confusionMatrix(xtab)
result<- confusionMatrix(predict_Dealth1, truth)
...
# Model Evaluation - Choosing K 
# Calculate out of Sample error 
misClassError15<- mean(classifier_knn15 != test$DEATH_EVENT) 
print(paste('Accuracy =', 1-misClassError15)) 


   conm15<-table(classifier_knn15,test$DEATH_EVENT)
   conm15
  lvs <- c("0", "1")
truth<- factor(rep(lvs, times = c(32, 32)),
                levels = rev(lvs))
predict_Dealth1<- factor(
               c(
                 rep(lvs, times = c(27, 8)),
                 rep(lvs, times = c(5, 24))),               
               levels = rev(lvs))
xtab <- table(predict_Dealth1, truth)
caret::confusionMatrix(xtab)
result<- confusionMatrix(predict_Dealth1, truth)
...
# Model Evaluation - Choosing K 
# Calculate out of Sample error 
misClassError19<- mean(classifier_knn19!= test$DEATH_EVENT) 
print(paste('Accuracy =', 1-misClassError19))
  conm19<-table(classifier_knn19,test$DEATH_EVENT)
   conm19
lvs <- c("0", "1")
truth<- factor(rep(lvs, times = c(32, 32)),
                levels = rev(lvs))
predict_Dealth1<- factor(
               c(
                 rep(lvs, times = c(27, 9)),
                 rep(lvs, times = c(5, 23))),               
               levels = rev(lvs))
xtab <- table(predict_Dealth1, truth)
caret::confusionMatrix(xtab)
result<- confusionMatrix(predict_Dealth1, truth)
...
table(test_HeartData$DEATH_EVENT)
...
y_pred<- factor(
               c(
                 rep(lvs, times = c(26,6)),
                 rep(lvs, times = c(6, 26))),               
               levels = rev(lvs))
xtabsvm<- table(y_pred, truth)
caret::confusionMatrix(xtabsvm)
resultsvm<- confusionMatrix(y_pred, truth)
resultsvm
...
#table(train$DEATH_EVENT)
 ...
 table(predict_Dealth,test$DEATH_EVENT)
lvs <- c("0", "1")
trutho <- factor(rep(lvs, times = c(39, 39)),
                levels = rev(lvs))
predict_Dealth <- factor(
               c(
                 rep(lvs, times = c(33, 6)),
                 rep(lvs, times = c(6, 33))),               
               levels = rev(lvs))
xtab <- table(predict_Dealth, trutho)
caret::confusionMatrix(xtab)
result<- confusionMatrix(predict_Dealth, trutho)
result
precision<- result$byClass['Pos Pred Value']
precision
recall<- result$byClass['Sensitivity']
...
table(test$DEATH_EVENT, predict_Dealth1)
lvs <- c("0", "1")
truth<- factor(rep(lvs, times = c(39, 39)),
                levels = rev(lvs))
predict_Dealth1<- factor(
               c(
                 rep(lvs, times = c(33, 6)),
...
train7<- subset(HeartData_IMP, split == "TRUE") 
test7<- subset(HeartData_IMP, split == "FALSE") 

opt_model7<- glm(DEATH_EVENT~.,data=train7,family = binomial(logit), maxit = 100) 
opt_model7
# Summary 
summary(opt_model7) 
   opt_model7%>% stepAIC(trace = FALSE)
coef(opt_model7)
summary(opt_model7)$coef
# Predict test7 data based on model 




predict_Dealth7 <-predict(opt_model7,test7, type = "response") 
predict_Dealth7 
   probabilities <- opt_model7%>% predict(test7, type = "response")
# Changing probabilities 
predict_Dealth7<-ifelse(probabilities > 0.5, 1, 0)
# Evaluating model accuracy 
# using confusion matrix 
table(test7$DEATH_EVENT,predict_Dealth7) 
   
classerr <- mean(predict_Dealth!= test7$Dealth) 
#print(paste('Accuracy =', 1 -classerr)) 
 mean(predict_Dealth7==test7$DEATH_EVENT)
table(predict_Dealth7==test7$DEATH_EVENT)  
# ROC-AUC Curve 
ROCPred7<-ROCR::prediction(predict_Dealth7,test7[,8])  
ROCPer <-ROCR::performance(ROCPred,"tpr", x.measure = "fpr") 
...
legend(.6, .4, auc, title = "AUC", cex =0.3)
   ...
   #confusion matrix and accuracy
table(test7$DEATH_EVENT)
table(test7$DEATH_EVENT, predict_Dealth7)
lvs <- c("0", "1")
truth<- factor(rep(lvs, times = c(39, 39)),
                levels = rev(lvs))
predict_Dealth7<- factor(
               c(
                 rep(lvs, times = c(34, 5)),
                 rep(lvs, times = c(7, 32))),               
               levels = rev(lvs))
xtab <- table(predict_Dealth7,truth)
caret::confusionMatrix(xtab)
result<- confusionMatrix(predict_Dealth7, truth)
result
precision<- result$byClass['Pos Pred Value']
precision
recall<- result$byClass['Sensitivity']
